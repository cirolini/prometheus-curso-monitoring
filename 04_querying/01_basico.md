...
O `offset` é sempre relativo: "5 minutos atrás" em relação ao momento da consulta. Quando você quer um instante fixo, existe o modificador `@`, que recebe um timestamp Unix:

```
prometheus_http_requests_total @ 1609459200
```

Isso é bem útil em alerta e em comparação, porque o valor não muda conforme o tempo passa. Dá para combinar os dois, e a ordem não importa:

```
prometheus_http_requests_total @ 1609459200 offset 1d
```

Tem ainda dois atalhos que economizam conta: `@ start()` e `@ end()`, que fixam no começo e no fim da janela que você está consultando.
