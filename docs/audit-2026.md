...
- **Não consegui rodar nenhum lab.** Esta máquina não tem `docker`, `kind`, `helm` nem `kubectl` instalados (só `make`, `python3` e `uv`). A regra 4 do teu plano — "todo lab testado de verdade antes do PR" — **não tem como ser cumprida aqui como está**. Os bloqueadores B1, B2 e o comportamento do Prometheus 3 eu validei baixando e executando o binário `prometheus-3.14.0.darwin-arm64` localmente; o resto do módulo 07 é análise estática.
- **Antes da Fase 1 preciso que tu instale** pelo menos Docker Desktop (ou colima/podman), `kind`, `kubectl` e `helm`. Ou que me digas para seguir sem teste de execução — mas aí isso precisa ficar registrado no PR.
